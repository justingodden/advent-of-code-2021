...
for (let uX = 1; uX <= maxX; uX++) {
    ...
    for (let uY = minY; uY < 1000; uY++) {
        // initial x and y position, track higest y and if we got into the target zone
        let x = 0
        let y = 0
        let highestY = 0
        let target = false
        ...
        // if we got into the target zone and got a new best higest y
        if (target && highestY > overallHighestY) {
            overallHighestY = highestY
        }
    }
}

console.log(overallHighestY)